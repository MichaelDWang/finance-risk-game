...
describe('mail validation and provider states (no real sends)',()=>{
  ...
  it('recomputes, fixes the template and sends only to the requested recipient',async()=>{const t=setup(),d=t.data();const res=await t.handler(t.request(d),t.env);expect(res.status).toBe(202);expect(await res.json()).toEqual({status:'accepted'});expect(t.generate.mock.calls[0][0].score).toBeCloseTo(54.16667);expect(t.generate.mock.calls[0][0].dateHK).toBe(buildReport(d.session).dateHK);const payload=JSON.parse(String(t.sent[0].body));expect(payload.to).toEqual([d.email]);expect(payload.from).toBe(t.env.FROM_EMAIL);expect(payload.attachments).toHaveLength(1);expect(payload.text).toContain('認識財務金融系');expect(payload.subject).toContain('金融探索報告');});
